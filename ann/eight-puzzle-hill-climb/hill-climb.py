class EightPuzzleHillClimbing:
    ...
    def display_state(self, state):
        """show the current puzzle state in a 3x3 grid."""
        for row in state:
            print(row)
        print()

    def get_possible_moves(self, state):
        """get possible states after moving the blank tile (0) up, down, left, or right."""
        possible_moves = []
    ...
    def heuristic(self, state):
        """count the number of misplaced tiles (excluding the blank tile)."""
        misplaced_tiles = 0
        for i in range(3):
            for j in range(3):
